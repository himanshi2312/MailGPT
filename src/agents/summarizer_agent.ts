...
export const summariserAgent = async (
  ...
  const analystLLM = llm.bindTools([documentTool]);
  ...
  if (!data.analystFinished) {
    ...
    const analystResult = await analystLLM.invoke(data.summariser_msgs, {
      tool_choice: "documentTool",
    });
    ...
    return new Command({
      goto: "tool_node",
      update: {
        summariser_msgs: [analystResult],
      },
    });
  } else {
    // If the Analyst is finished, we pass the final content to the Supervisor
    const finalAnalystMessage =
      data.summariser_msgs[data.summariser_msgs.length - 1];
